...
const AdminDashboard: React.FC = () => {
  ...
  const [carouselSlides, setCarouselSlides] = useState<CarouselSlide[]>([
    { id: 1, src: "/flyer1.jpg", link: "/evento/1", active: true, title: "Evento 1" },
    { id: 2, src: "/flyer2.jpeg", link: "/evento/2", active: true, title: "Evento 2" },
    { id: 3, src: "/flyer3.jpeg", link: "/evento/3", active: true, title: "Evento 3" },
    { id: 4, src: "/flyer4.webp", link: "/evento/4", active: false, title: "Evento 4" },
    { id: 5, src: "/background-login.png", link: "/evento/5", active: true, title: "Evento 5" },
  ])
  ...
  const addSlide = () => {
    const newSlide: CarouselSlide = {
      ...
      link: "",
      active: true,
      title: `Novo Slide ${carouselSlides.length + 1}`,
    }
  ...
  const updateSlide = (id: number, field: keyof CarouselSlide, value: any) => {
    setCarouselSlides(carouselSlides.map((slide) => (slide.id === id ? { ...slide, [field]: value } : slide)))
    setHasChanges(true)
  }

  const toggleSlideActive = (id: number) => {
    updateSlide(id, "active", !carouselSlides.find((s) => s.id === id)?.active)
  }

  const updateBanner = (field: keyof BannerData, value: any) => {
  ...
  const handleSave = async () => {
    try {
      console.log("Salvando alterações...", { carouselSlides, bannerData })
      setHasChanges(false)
    ...
    input.onchange = (e) => {
      ...
      if (file) {
        ...
        reader.onload = (e) => {
          ...
          if (slideId) {
            updateSlide(slideId, "src", imageUrl)
          } else {
        ...
        <div className="max-w-7xl mx-auto space-y-4 sm:space-y-6">
          <div className="bg-white rounded-lg shadow-sm border p-4 sm:p-6">
            <div className="block sm:hidden space-y-4">
              <div>
                <h1 className="text-xl font-bold text-gray-900">Dashboard de Propagandas</h1>
              </div>

              <div>
                <p className="text-gray-600 text-sm">Gerencie carrossel e banners da página inicial</p>
              </div>

              <div className="flex flex-col gap-2">
                {hasChanges && (
                  <Button variant="outline" onClick={handleReset} className="cursor-pointer gap-2 text-sm w-full">
            ...
            <div className="hidden sm:block">
              <div className="space-y-4">
                <div>
                  <h1 className="text-xl sm:text-2xl font-bold text-gray-900">Dashboard de Propagandas</h1>
                ...
                <div className="flex flex-col sm:flex-row gap-2 sm:gap-3">
                  {hasChanges && (
                    <Button variant="outline" onClick={handleReset} className="cursor-pointer gap-2 text-sm w-full sm:w-auto">
                      <RotateCcw className="w-4 h-4" />
                      Descartar
                    </Button>
                  )}
          ...
          <Tabs defaultValue="carousel" className="space-y-4 sm:space-y-6">
            <TabsList className="grid w-full grid-cols-2 max-w-full sm:max-w-md h-auto">
              <TabsTrigger value="carousel" className="cursor-pointer gap-1 sm:gap-2 text-xs sm:text-sm py-2 sm:py-3">
                <Settings className="w-3 h-3 sm:w-4 sm:h-4" />
                <span>Carrossel</span>
              </TabsTrigger>
              <TabsTrigger value="banner" className="cursor-pointer gap-1 sm:gap-2 text-xs sm:text-sm py-2 sm:py-3">
            ...
            <TabsContent value="carousel" className="space-y-4 sm:space-y-6">
              <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
                <div>
                  <h2 className="text-lg sm:text-xl font-semibold">Gerenciar Carrossel</h2>
                  <p className="text-gray-600 text-sm sm:text-base">Configure os slides do carrossel principal</p>
                </div>
                <Button onClick={addSlide} className="gap-2 w-full sm:w-auto cursor-pointer">
                      ...
                      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3">
                        <div className="flex items-center gap-3">
                          ...
                          <div className="min-w-0 flex-1">
                            <CardTitle className="text-base sm:text-lg truncate">Slide {index + 1}</CardTitle>
                        ...
                        <div className="flex items-center gap-2 flex-wrap">
                          ...
                          <div className="flex gap-1">
                            <Button variant="ghost" size="sm" onClick={() => toggleSlideActive(slide.id)}>
                              {slide.active ? <Eye className="w-4 h-4 cursor-pointer" /> : <EyeOff className="w-4 h-4" />}
                            </Button>
                            <Button
                              variant="ghost"
                              size="sm"
                      ...
                      <div className="grid grid-cols-1 lg:grid-cols-3 gap-4">
                        ...
                        <div className="lg:col-span-2 space-y-4">
                          <div className="space-y-2">
                            <Label className="text-sm font-medium">Título do Slide</Label>
                            <Input
                              value={slide.title || ""}
                          ...
                          <div className="space-y-2">
                            <Label className="text-sm font-medium">Link de Redirecionamento</Label>
                          ...
                          <div className="flex items-center space-x-2 pt-2">
                            <Switch checked={slide.active} onCheckedChange={() => toggleSlideActive(slide.id)} />
            ...
            <TabsContent value="banner" className="space-y-4 sm:space-y-6">
              <div>
                <h2 className="text-lg sm:text-xl font-semibold">Gerenciar Banner</h2>
                <p className="text-gray-600 text-sm sm:text-base">Configure o banner de propaganda principal</p>
              </div>
                ...
                <CardHeader className="p-4 sm:p-6">
                  <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3">
                    <div className="min-w-0">
                      <CardTitle className="text-base sm:text-lg">Banner Principal</CardTitle>
                      <CardDescription className="text-sm">Banner exibido na página inicial</CardDescription>
                    </div>
                    <Badge variant={bannerData.active ? "default" : "secondary"} className="text-xs w-fit">
                    ...
                    <div className="space-y-4">
                      ...
                      <div className="flex items-center space-x-2 pt-2">
                        <Switch
                          ...
                          onCheckedChange={(checked) => updateBanner("active", checked)}
                        />
              ...
              <CardContent className="pt-4 sm:pt-6 p-4 sm:p-6">
                <div className="flex flex-col sm:flex-row sm:items-center gap-2 text-yellow-800">
                  <div className="flex items-center gap-2">
                    ...
                    <span className="font-medium text-sm sm:text-base">Você tem alterações não salvas</span>
                  </div>
                </div>
